...
def _stub_item_modules(stock=None, cross=None, stock_edit_result=True, cross_edit_result=True):
    ...
    access_module = ModuleType("flymanager.utils.mongo.access")
    access_module.get_accessible_stock = MagicMock(return_value=stock)
    access_module.get_accessible_cross = MagicMock(return_value=cross)
    ...
    modules = {
        ...
        "flymanager.utils.mongo.access": access_module,
        "flymanager.utils.mongo.stocks": stock_module,
        "flymanager.utils.mongo.crosses": cross_module,
    }
    return modules, access_module, stock_module, cross_module


def test_move_item_to_tray_allows_reposition_within_same_tray():
    db = MagicMock()
    tray = {"UniqueID": "tray-1", "User": "alice", "TrayID": "TRAY-1", "Rows": 10, "Columns": 10}
    stock = {
        "UniqueID": "STK-1",
        "User": "alice",
        "TrayID": "TRAY-1",
    ...
    modules, access_module, stock_module, _ = _stub_item_modules(stock=stock)

    with patch.object(trays_module, "get_accessible_tray", return_value=tray), patch.object(
    ...
    access_module.get_accessible_stock.assert_called_once_with("alice", "STK-1", db, annotate=True)
...
def test_move_item_to_tray_rejects_conflict_with_other_item():
    ...
    modules, _, stock_module, _ = _stub_item_modules(stock=stock)
...
def test_move_item_to_tray_rejects_moves_into_another_owners_tray():
    db = MagicMock()
    tray = {"UniqueID": "tray-2", "User": "bob", "TrayID": "TRAY-2", "Rows": 10, "Columns": 10}
    stock = {
        "UniqueID": "STK-1",
        "User": "alice",
        "TrayID": "TRAY-1",
    ...
    with patch.object(trays_module, "get_accessible_tray", return_value=tray), patch.object(
        trays_module, "get_tray_occupancy", return_value={}
    ), patch.dict(sys.modules, modules):
        success = trays_module.move_item_to_tray("alice", "stock", "STK-1", "TRAY-2", "11", db)
