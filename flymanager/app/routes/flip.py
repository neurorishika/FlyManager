...
from flask import (
    ...
    jsonify, url_for, current_app, flash
)
...
from flymanager.utils.mongo import (
    get_user_stocks, get_user_crosses, flip_stock, flip_cross,
    get_flip_schedule, get_user_initials, write_activity
)
from flymanager.utils.labels import generate_label_pdf
from flymanager.app.routes.auth import login_required

# Define the Blueprint
bp = Blueprint('flip', __name__, url_prefix='/flip') # url_prefix defined in app/__init__
...
def flip_interface():
    ...
    return render_template('flip.html', username=username, ports=ports) # Add active_scan_info if needed
    ...
    try:
        ...
        if last_flip_str:
            try:
                ...
                try:
                    last_flip_dt = datetime.strptime(last_flip_str.split('.')[0], '%Y-%m-%d %H:%M:%S')
                except:
                    last_flip_dt = datetime.strptime(last_flip_str.split('.')[0], '%Y-%m-%d %H:%M')
                difference = (flip_time - last_flip_dt).total_seconds()
                if difference < MIN_FLIP_DIFFERENCE:
                    print(f'{item_type} {uid} already flipped recently ({difference:.0f}s ago)')
                    return jsonify({'message': f'{item_type} already flipped recently at: {last_flip_dt.strftime("%Y-%m-%d %H:%M:%S")}'}), 409 # 409 Conflict

            except (ValueError, TypeError) as e:
                print(f"Could not parse last flip date '{last_flip_str}' for {item_type.lower()} {uid}: {e}")
        ...
        flip_function(username, uid, db, flip_time, new_status=status, added_comment=comment)
            ...
            try:
                ...
                if stock:
                    ...
                    results['failed'].append({
                        'uid': uid,
                        'reason': 'UID not recognized for this user.'
                    })
                    continue
                    
                # Flip the item
                flip_function(username, uid, db, flip_time, new_status=status, added_comment=comment)
                write_activity(username, f'Flipped {item_type.lower()} {uid} (bulk operation)', db)
...
def bulk_status_change_route():
    ...
    try:
            ...
            try:
                ...
                if stock:
                    db['stocks'].update_one(
                        ...
                        {"$set": {"Status": status, "DataModifiedDate": datetime.now().strftime('%Y-%m-%d %H:%M:%S')}}
                    )
                    item_type = "Stock"
                elif cross:
                ...
                write_activity(username, f'Changed status of {item_type.lower()} {uid} to {status} (bulk operation)', db)
                results['success'].append({
                    'uid': uid,
                    'type': item_type
                })
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'message': 'An internal error occurred during bulk status change.'}), 500
...
def flip_schedule_display():
    ...
    username = session.get("username")
    try:
        schedule = get_flip_schedule(username, db) # Fetch raw schedule {date: [item_str, ...]}
        ...
        for date_str, items in sorted(schedule.items()): # Sort by date
            try:
                ...
                print(f"Warning: Invalid date format '{date_str}' in schedule for user {username}")
            ...
            for item_str in items:
                ...
                try:
                    ...
                    if id_part_start != -1:
                        ...
                        if id_part_end == -1: id_part_end = item_str.find(")", id_part_start) # Handle case with no comma after ID
                        if id_part_end != -1:
                    ...
                    if uid:
                        ...
                        if item_data:
                            ...
                            if tray_id:
                             ...
                             print(f"Warning: Data not found for scheduled {item_type or 'item'} UID {uid}")
                    ...
                    processed_items.append({
                     ...
                     print(f"Error parsing schedule item string '{item_str}': {parse_err}")
                     # Add item even if parsing fails partially
                     processed_items.append({'display_string': item_str, 'uid': None, 'type': None, 'tray': None, 'data': None})
        ...
        return render_template('flip_schedule.html', schedule={}, username=username, error="Could not generate schedule.")
    ...
    try:
        ...
        for item_str in items_for_date:
            ...
            try:
                ...
                if id_part_start != -1:
                    id_part_end = item_str.find(",", id_part_start)
                    if id_part_end == -1: id_part_end = item_str.find(")", id_part_start)
                    if id_part_end != -1:
                         uid = item_str[id_part_start + 5 : id_part_end].strip()
                ...
                if uid:
                    ...
                    if item_data:
                        ...
                        print(f"Warning: Data not found for scheduled {item_type or 'item'} UID {uid} during label generation.")
                else:
                     print(f"Could not extract UID from schedule item string: {item_str}")

            except Exception as parse_err:
                print(f"Error parsing schedule item string '{item_str}' for labels: {parse_err}")
        ...
        def sort_key(item):
            ...
            try:
                # Handle potential non-integer TrayPosition like '1.1'
                pos = int(str(item.get('TrayPosition', '0')).split('.')[0] or 0)
            except ValueError:
        ...
        sorted_items_data, sorted_item_types = zip(*paired_list) if paired_list else ([], [])
        ...
        # Log activity
        write_activity(username, f'Generated {len(sorted_items_data)} labels for {date_str}', db)
