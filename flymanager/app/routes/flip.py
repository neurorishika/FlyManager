...
def generate_labels_for_day_route():
    ...
    try:
        ...
        if not schedule.get(date_str):
            flash(f"No items scheduled for {date_str}.", "info")
            return redirect(url_for("flip.flip_schedule_display"))

        # Shared with the reminder email, which attaches the same PDF.
        pdf_bytes, item_count = build_label_pdf(
            username, [date_str], db, blank_spaces=blank_spaces
        )
        if not pdf_bytes:
        ...
        filename_base = f"{username}_labels_{date_str.replace('-', '')}_{timestamp}"
        ...
        with open(pdf_full_path, "wb") as handle:
        ...
        # Log activity
        write_activity(
            username, f"Generated {item_count} labels for {date_str}", db
        )
