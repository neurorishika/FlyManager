...
def add_to_cross(user, properties, db):
    ...
    uid = (
        str(user)
        + str(properties["MaleUniqueID"])
        + str(properties["FemaleUniqueID"])
        + str(properties["Name"])
    )
    ...
    if current_cross:
        ...
        if new_status and current_cross.get("Status") != new_status:
            ...
            modification_log_entries.append(
                f"{ts} : Status changed from {current_cross.get('Status')} to {new_status}"
            )
...
def update_cross_vials(cross, username, db):
    ...
    if "CurrentlyAliveVials" not in cross:
        ...
        next_flip_dates = [
            date
            for date in next_flip_dates
            if date <= first_flip_date + datetime.timedelta(days=maxCrossLifetime)
        ]
